...
int main() {
    ...
    light_pos = point3(0, 0.9, -1);  // Light at the ceiling
    
    // Scene    
    hittable_list world;
    
    // Materials
    auto wall_material = std::make_shared<noMaterial>();
    auto metal_material = std::make_shared<metal>(color(0.8, 0.8, 0.8));
    auto no_material = std::make_shared<noMaterial>();

    offmodel* model =  readOffFile("models/cube.off");  
 ...
 for (int i = 0; i < model->numberOfPolygons; ++i) {
    ...
    for (int j = 1; j < poly.noSides - 1; ++j) {
        ...
        world.add(std::make_shared<triangle>(v0, v1, v2, color(0.0,0.0,1.0)));
    }
}
   
    // Room
    // Floor (large cube at the bottom)
    world.add(std::make_shared<cube>(point3(0.0, -1.0, -1.5), 2.0, 0.1, 2.0, color(0.8, 0.8, 0.8)));
    
    // Ceiling (large cube at the top)
    world.add(std::make_shared<cube>(point3(0.0, 1.0, -1.5), 2.0, 0.1, 2.0, color(0.9, 0.9, 0.9)));
    
    // Left wall
    world.add(std::make_shared<cube>(point3(-1.0, 0.0, -1.5), 0.1, 2.0, 2.0, color(1.0, 0.5, 0.5)));
    
    // Right wall
    world.add(std::make_shared<cube>(point3(1.0, 0.0, -1.5), 0.1, 2.0, 2.0, color(0.5, 0.5, 1.0)));
    
    // Back wall
    world.add(std::make_shared<cube>(point3(0.0, 0.0, -2.5), 2.0, 2.0, 0.1, color(0.9, 0.9, 0.9)));
    
    // Objects in the room
    // Metal sphere on the floor
    world.add(std::make_shared<sphere>(point3(-0.5, -0.75, -1.5), 0.25, color(0.8, 0.6, 0.2), metal_material));
    
    // Cube on the floor with no material
    world.add(std::make_shared<cube>(point3(0.5, -0.75, -1.2), 0.25, 0.25, 0.25, color(0.2, 0.8, 0.2)));
    ...
    for (int j = image_height - 1; j >= 0; --j) {
        ...
        for (int i = 0; i < image_width; ++i) {
            ...
            color pixel_color = ray_color(r, world); // Or whatever depth value is appropriate
